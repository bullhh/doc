...
- modules/axhal/src/arch/aarch64/hv: 用于定义异常处理上下文的结构，以及异常向量表的定义。
...
- modules/axruntime/src/hv/aarch64_kernel：定义了虚拟设备相关的处理函数，具体的异常处理函数，以及多核启动情况下的psci的处理函数（多核启动暂时用不上）
## 1.2 建立执行环境
arceOS本身的启动与初始化流程在modules/axhal/src/platform/aarch64_common/boot.rs中。
1. 初始化EL2的异常向量表
2. 初始化栈帧
3. 初始化页表以及EL2的页表基址寄存器，开启MMU
完成设备基本初始化后会跳转到modules/axruntime/src/lib.rs中rust_main执行。初始化内存分配器后会跳转到apps/hv/main.rs中main函数执行，在其中启动虚拟机。
## 1.3 运行
